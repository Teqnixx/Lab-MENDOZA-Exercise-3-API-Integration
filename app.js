document.addEventListener("DOMContentLoaded", function() {
    const apiKey = "EUDKZNLABA7nQ0UJdGEnLoTbzripqnX3"; // Use provided API key
    ...
    function displayDailyForecast(forecasts) {
        ...
        forecasts.forEach(forecast => {
            const date = new Date(forecast.Date).toLocaleDateString('en-us', options);
            const weekday = new Date(forecast.Date).toLocaleDateString('en-us', options2)
        ...
        forecasts.forEach(forecast => {
            ...
            forecastContent += `
                <div class="forecast-hourly-item">
                    <img src="https://developer.accuweather.com/sites/default/files/${String(weatherIcon).padStart(2, '0')}-s.png" alt="${weatherText}">
                    <div class="forecast-hourly-content">
                        <p>${time}</p>
                        <p>Temperature: ${temperature}°C</p>
                        <p>${weatherText}</p>
                    </div>
